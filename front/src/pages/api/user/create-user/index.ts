import { createUser } from '@/utils/axios/user/createUser';
...
export default async function createUserBFF(
	req: NextApiRequest,
	...
	try {
		...
		if (req.method === 'POST') {
			...
			// Calling the createUser function with the provided email and password
			const user = await createUser({ email: email, password: password });

			// Sending the response with the status and data from the createUser result
